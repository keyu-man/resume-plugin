...
description: 当你需要把任意排版的简历 PDF 转换为本仓库的标准 DOCX 输出时使用。本技能会提取 PDF 中可选中文本 + 图片内文字（OCR）到 Markdown，再将 Markdown 映射到 template/data_template.json（参考 template/data_demo.json 的风格），规范化 template/data.json，最后运行 `openclaw resume parse` 渲染最终 DOCX。
---
...
2. 将 PDF 提取为 Markdown：包含原生 PDF 文本 + OCR 文本。
